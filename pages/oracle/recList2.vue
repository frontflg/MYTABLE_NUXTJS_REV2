    ...
    async searchData () {
      try {
        ...
        for (let i = 0; i < this.fields.length; i++) {
          ...
          if (this.fields[i].type === 10) { // date
            widthSize = 110
          } else if (this.fields[i].type === 11) { // time
            widthSize = 100
          } else if (this.fields[i].type === 12) { // datetime
            widthSize = 120
          } else if (this.fields[i].type === 7) { // timestamp
            widthSize = 120
            ...
            try {
              ...
              if (str !== null) {
                switch (this.fields[i].type) {
                  case 10:
                    this.lists[item][subItem] = this.$dayjs(new Date(Date.parse(str))).locale('ja').format('YYYY-MM-DD')
                    break
                  case 7:
                  case 12:
                    this.lists[item][subItem] = this.$dayjs(new Date(Date.parse(str))).locale('ja').format('YYYY-MM-DD HH:mm:ss')
                }
              }
            } catch (e) { } // 握りつぶす
            i++
        ...
        const sql2 = 'SELECT C.COMMENTS,T.COLUMN_NAME FROM USER_TAB_COLUMNS T,USER_COL_COMMENTS C' +
          ' WHERE T.TABLE_NAME ="' + this.inTblId + '" AND T.TABLE_NAME = C.TABLE_NAME' +
          ' AND T.COLUMN_NAME = C.COLUMN_NAME ORDER BY T.COLUMN_ID'
        const res2 = await this.$axios.$get('/api?sql=' + sql2)
        this.fields2 = res2[0]
        for (let i = 0; i < this.fields2.length; i++) {
          if (typeof this.headers[i] !== 'undefined') {
            if (this.fields2[i].COLUMN_COMMENT && this.headers[i].text === this.fields2[i].COLUMN_NAME) {
