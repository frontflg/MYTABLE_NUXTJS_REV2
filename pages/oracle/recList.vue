...
export default {
  ...
  async fetch () {
    let sql = 'SELECT CASE WHEN C.COMMENTS = NULL OR C.COMMENTS = ""'
    sql += ' THEN T.COLUMN_NAME ELSE C.COMMENTS END AS TEXT,T.COLUMN_NAME AS VALUE,'
    sql += ' T.DATA_LENGTH * 7 AS WIDTH,T.DATA_TYPE AS DATATYPE,'
    sql += ' T.DATA_LENGTH AS DATALENG,T.NULLABLE AS NULLABL'
    sql += ' FROM USER_TAB_COLUMNS T,USER_COL_COMMENTS C'
    sql += ' WHERE T.TABLE_NAME ="' + this.inTblId + '" AND T.TABLE_NAME = C.TABLE_NAME'
    sql += ' AND T.COLUMN_NAME = C.COLUMN_NAME ORDER BY T.COLUMN_ID'
